...
public class Post {
    ...
    // can't be null - optional annotation that allows custom mapping between entity field and database column
    ...
    // mappedBy post indicates that it is the post column from Comments entity that maps to this post entity. The cascade = CascadeType.REMOVE attribute specifies that when a Post entity is removed, all related Comment entities should also be removed (cascaded deletion).
    private Set<Comment> comments = new HashSet<>();
    @ManyToOne // many posts belong to one user
    @JoinColumn(name = "created_by", nullable = false)
    // column that is used to hold the foreign keys for the post entity
